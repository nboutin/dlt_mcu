...
bool DLT_datalink_serial_copy_log(dlt_context_t* context, DLT_Frame_t* frame)
{
  ...
  else
  {
    ...
    if (DLT_mutex_acquire() == true)
    {
      ...
      uint16_t frame_size = DLT_SERIAL_HEADER_SIZE + DLT_HEADER_SIZE + BUF_GetUsedSize(&frame->payload);
      if ((frame_size < DLT_FRAME_SIZE_MAX) && (RBUF_GetFreeSize(&g_tx_buffer) >= frame_size))
      {
        copied = true;

        // Serial header (DLS1)
        copied &= RBUF_WriteString(&g_tx_buffer, DLT_SERIAL_HEADER, DLT_SERIAL_HEADER_SIZE);
        // Frame Header
        copied &= DLT_frame_make_header(&g_tx_buffer,
                                        frame->msg_counter,
                                        frame->timestamp,
                                        frame->msg_id,
                                        DLT_HEADER_SIZE + BUF_GetUsedSize(&frame->payload));
        // Payload
        copied &= RBUF_WriteCopy(&g_tx_buffer, &frame->payload, BUF_GetToReadCount(&frame->payload));

        if (copied == true)
        {
          // DLT_Stat_report_datalink(DLT_STAT_DATALINK_SERIAL,
          // DLT_STAT_DATALINK_COPIED, frame_size);
        }
      }
      else
      {
      ...
      // DLT_Stat_report_usage(DLT_STAT_DATALINK_SERIAL,
      // RBUF_GetUsedSize(&g_tx_buffer), DLT_SERIAL_TX_BUFFER_SIZE);
...
void DLT_datalink_serial_transmit()
{
  ...
  if (DLT_mutex_acquire() == true)
  {
    ...
    {
      BUF_Reset(&g_tx_chunk_buffer);

      g_last_transfer_size = RBUF_ReadCopyRaw(&g_tx_chunk_buffer, &g_tx_buffer, DLT_SERIAL_TX_CHUNK_SIZE_MAX);
      ...
      if (g_last_transfer_size > 0U)
      {
        ...
        {
          // DLT_Stat_report_datalink(DLT_STAT_DATALINK_SERIAL,
          // DLT_STAT_DATALINK_TRANSMITTED, g_last_transfer_size);
        }
      }
    }
    DLT_mutex_release();
